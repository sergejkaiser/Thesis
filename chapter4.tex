...
Since the trade relationships in each trade network are not symmetric the trade network is a directed network.%, as a country may be exporting in a partiuclar industry to a second country but may be not import from that country in the particular industrz or vice a versa,
 \par
...
Analog to the matrices of trade relationships for the binary trade networks, I define for each industry $k$ a matrix, which records the weight of each trade relationship.
  This matrix $W^k$ is denoted as weight matrix. %Further, it has the interpretation of recording  the equilibrium relations ater interactions have taken place \parencite{de2010}. \par
