...
%why do we care
%similarity of network centrality to predictions of micro Ricardo model. Lower realtive cost to produce a good, hence higher trade shares.
...
Each edge $g_{i,j}^k $ represents a trade relation between an industry $k$ in exporting country $i$ to importing country $j$.
...
To simplify we represent the weight variable, in $k$  matrices $W^k$, which I denote as weight matrices. %Further, it has the interpretation of recording  the equilibrium relations after interactions have taken place \parencite{de2010}. \par
